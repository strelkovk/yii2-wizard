<?php
     ...
     *
     * @author    Chris Yates
     * @copyright Copyright &copy; 2015 strelkovk - All Rights Reserved
     * @license   BSD 3-Clause
     * @package   Wizard
     */

    namespace strelkovk\wizard;
    ...
    class WizardBehavior extends Behavior
    {
        ...
        const BRANCH_SKIP      = -1;
        const BRANCH_SKIP_STEP = 2;
        const BRANCH_ON_STEP   = 3;
                ...
                if ($event->continue) {
                    if ($event->handled) {
                        if ($event->nextStep !== self::DIRECTION_BACKWARD) {
                            $this->saveStep($step, $event->data instanceof Model ? $event->data->getAttributes() : $event->data);
                    ...
                    if ($event->handled) {
                        //$this->saveStep($step, $event->data->attributes());
                        $this->saveStep($step, $event->data instanceof Model ? $event->data->getAttributes() : $event->data);
         ...
         */
        public function getSteps($full = false, $onlyValid = false)
        {
            if($onlyValid){
                $steps = [];
                $keys = array_keys($this->_session[$this->_stepsKey]);
                foreach ($this->_stepsConfig as $key => $value) {
                ...
                return $full ? $this->_stepsConfig : array_keys($this->_session[$this->_stepsKey]);
            }
        }

        public function getAvaliableSteps($full = false)
        {
            return $full ? $this->_stepsConfig : array_keys($this->_session[$this->_stepsKey]);
        }
        ...
        protected function branch($directives)
        {
            ...
            foreach ($directives as $name => $directive) {
                ...
                } elseif ($directive === self::BRANCH_SKIP_STEP) {
                    $this->_session[$this->_branchKey][$name] = self::BRANCH_SKIP_STEP;
                } elseif ($directive === self::BRANCH_ON_STEP) {
                    $this->_session[$this->_branchKey][$name] = self::BRANCH_ON_STEP;
                } else {
                    if (is_int($name)) {
                        $name      = $directive;
        ...
        private function _parseSteps($steps)
        {
            ...
            foreach ($steps as $label => $step) {
                ...
                if(isset($this->_session[$this->_branchKey][$label])
                    && $this->_session[$this->_branchKey][$label] == self::BRANCH_SKIP_STEP){
                    continue;
                }
